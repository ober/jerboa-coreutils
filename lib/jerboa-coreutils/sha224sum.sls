#!chezscheme
;;; sha224sum.sls -- SHA224 hash

(library (jerboa-coreutils sha224sum)
  (export main)
  ...
  (def (parse-check-line line)
    (let ((len (string-length line)))
      (cond
        ((and (>= len 7) (string-prefix? "SHA224 (" line))
  ...
  (def (main . args)
    (parameterize ((program-name "sha224sum"))
      (call-with-getopt
        (lambda (_ opt)
        ...
        args
        'program: "sha224sum"
        'help: "Print or check SHA224 message digests."
        (flag 'binary "-b" "--binary"
          'help: "read in binary mode")
        (flag 'check "-c" "--check"
          'help: "read SHA224 sums from the FILEs and check them")
        (flag 'tag "--tag"
