#!chezscheme
;;; sha1sum.sls -- SHA1 hash

(library (jerboa-coreutils sha1sum)
  (export main)
  ...
  (def (main . args)
    (parameterize ((program-name "sha1sum"))
      (init-security!)
      ...
      (call-with-getopt
        ...
        args
        'program: "sha1sum"
        'help: "Print or check SHA1 message digests."
        (flag 'binary "-b" "--binary"
          'help: "read in binary mode")
        (flag 'check "-c" "--check"
          'help: "read SHA1 sums from the FILEs and check them")
        (flag 'tag "--tag"
