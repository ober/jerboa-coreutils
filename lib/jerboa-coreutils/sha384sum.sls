#!chezscheme
;;; sha384sum.sls -- SHA384 hash

(library (jerboa-coreutils sha384sum)
  (export main)
  ...
  (def (parse-check-line line)
    (let ((len (string-length line)))
      (cond
        ((and (>= len 7) (string-prefix? "SHA384 (" line))
  ...
  (def (main . args)
    (parameterize ((program-name "sha384sum"))
      (init-security!)
      ...
      (call-with-getopt
        ...
        args
        'program: "sha384sum"
        'help: "Print or check SHA384 message digests."
        (flag 'binary "-b" "--binary"
          'help: "read in binary mode")
        (flag 'check "-c" "--check"
          'help: "read SHA384 sums from the FILEs and check them")
        (flag 'tag "--tag"
